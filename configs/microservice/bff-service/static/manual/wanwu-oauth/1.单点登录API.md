...
（1）在`.env.example`文件能找到如下三行OAuth配置，将其复制到`.env`文件中
...
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 \
  -out ~/.ssh/wanwu_rsa_private.pem

# 提取对应公钥
openssl rsa -pubout -in ~/.ssh/wanwu_rsa_private.pem \
  -out ~/.ssh/wanwu_rsa_public.pem
```
