...
（1）在`.env.example`文件能找到如下两行OAuth配置，将其复制到`.env`文件中

```shell
WANWU_BFF_OAUTH_SWITCH=0 //OAuth开关，0为关，1为开
WANWU_BFF_OAUTH_RSA_PATH=${WANWU_PROJECT_DIR}/rsa/oauth-rsa //RSA密钥目录
```
...
服务启动时如果密钥文件不存在会自动生成，无需手动操作。

**方案二：用户指定密钥**

使用以下命令在自定义目录中手动生成RSA密钥对：

```bash
# 创建密钥目录
mkdir -p ~/.ssh/wanwu-oauth-rsa

# 生成私钥（PKCS8格式）
openssl genpkey -algorithm RSA -out ~/.ssh/wanwu-oauth-rsa/oauth_rsa_private.pem -pkeyopt rsa_keygen_bits:2048

# 导出公钥
openssl rsa -in ~/.ssh/wanwu-oauth-rsa/oauth_rsa_private.pem -pubout -out ~/.ssh/wanwu-oauth-rsa/oauth_rsa_public.pem
```

然后在`.env`文件中修改 `WANWU_BFF_OAUTH_RSA_PATH` 指向该目录：

```shell
WANWU_BFF_OAUTH_RSA_PATH=~/.ssh/wanwu-oauth-rsa
```

密钥文件名需分别为 `oauth_rsa_private.pem`（私钥）和 `oauth_rsa_public.pem`（公钥）。
