...
| 解析字段 | 解析类型 | 解析样值 | → | 对象类字段 | 对象类类型 | 是否必填 | 差异类型 |
|---|---|---|---|---|---|---|---|
| 订单号 | STRING | O001 | → | `order_id` | STRING | 是 | 名称不一致 |
| 数量 | STRING("2") | 2 | → | `quantity` | INT | 是 | 名称不一致 + 类型不一致 |
| 金额 | STRING("12000") | 12000 | → | `amount` / `total_price`? | DECIMAL | 是 | 歧义 |
| 客户名称 | STRING | 张三 | → | `customer_name` | STRING | 否 | 一致（无需确认） |
